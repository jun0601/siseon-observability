...
본 레포는 StockOps의 관측(Observability) 데이터 파이프라인을 Terraform으로 관리한다. 두 가지 모듈로 구성된다.
...
EKS 위 앱 파드(api/ai)의 stdout 로그 → Fluent Bit(DaemonSet)이 수집 → CloudWatch Logs로 서비스별 전송.
...
> **역할 분리**: 본 레포는 **데이터 파이프라인(수집·적재)** 까지 책임진다. Grafana 데이터소스 연결 및 대시보드는 `siseon-infra-monitoring` 레포에서 관리한다. (Grafana는 단일 창구로 운영)
...
├── main.tf                  # provider, 모듈 호출 (iot_pipeline + app_logging)
...
└── modules/
    ...
    │   ├── variables.tf
    │   └── outputs.tf
    └── app_logging/         # CloudWatch Log Group, Fluent Bit(Helm), IRSA
        ├── main.tf
        ├── variables.tf
        └── outputs.tf
```
...
| 리소스 | 값 |
|--------|-----|
| CloudWatch Log Group (api) | `/aws/eks/seoul-cluster/stockops/api` (보존 7일) |
| CloudWatch Log Group (ai) | `/aws/eks/seoul-cluster/stockops/ai` (보존 7일) |
| Fluent Bit | DaemonSet, `amazon-cloudwatch` 네임스페이스 |
| Fluent Bit IRSA Role | `seoul-fluentbit-role` |
